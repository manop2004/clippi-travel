...
const dict: Record<string, Record<Lang, string>> = {
  ...
  // ===== ชุด A: หน้าที่นักท่องเที่ยว/ผู้ใช้ทั่วไปเห็น =====
  // -- ProtectedRoute / BannedGuard --
  "protect.checking":   { en: "Checking access permissions...", th: "กำลังตรวจสอบสิทธิ์การเข้าถึง...", jp: "アクセス権限を確認中..." },
  "protect.title":      { en: "Access Restricted",    th: "จำกัดการเข้าถึง",            jp: "アクセス制限" },
  "protect.desc":       { en: "You do not have permission to access this page. Required role:", th: "คุณไม่มีสิทธิ์เข้าถึงหน้านี้ ต้องมีสิทธิ์:", jp: "このページにアクセスする権限がありません。必要な権限：" },
  "protect.goHome":     { en: "Return to Explore Page", th: "กลับไปหน้าสำรวจ",          jp: "さがす画面へ戻る" },
  "ban.checking":       { en: "Verifying access...",   th: "กำลังตรวจสอบสิทธิ์การใช้งาน...", jp: "アクセス権を確認中..." },
  "ban.title":          { en: "Your account has been suspended", th: "บัญชีของคุณถูกระงับการใช้งาน", jp: "アカウントが停止されました" },
  "ban.desc":           { en: "You have been suspended and can no longer access any part of the system.", th: "คุณถูกระงับสิทธิ์การใช้งานระบบ ไม่สามารถเข้าถึงส่วนใดๆ ได้", jp: "利用が停止され、システムにアクセスできません。" },
  "ban.reasonLabel":    { en: "Reason for ban:",       th: "สาเหตุการแบน:",              jp: "停止理由：" },
  "ban.defaultReason":  { en: "Violation of the system's terms of use", th: "ละเมิดเงื่อนไขการใช้งานระบบ", jp: "利用規約違反" },
  // -- Carousel --
  "carousel.prev":      { en: "Slide left",            th: "สไลด์ไปทางซ้าย",            jp: "左へスライド" },
  "carousel.next":      { en: "Slide right",           th: "สไลด์ไปทางขวา",             jp: "右へスライド" },
  // -- ShopVersionHistory --
  "ver.validUntil":     { en: "Valid until:",          th: "กำหนดระยะเวลา:",            jp: "有効期限：" },
  "ver.noStampYet":     { en: "You haven't collected a stamp from this shop yet. Check in to get your first one!", th: "คุณยังไม่เคยสะสมแสตมป์ร้านนี้ ออกเดินทางเช็คอินเพื่อรับแสตมป์รอบแรกได้เลย!", jp: "この店のスタンプはまだありません。チェックインして最初の1つを獲得しましょう！" },
  "ver.collected":      { en: "This version collected!", th: "สะสมเวอร์ชันนี้แล้ว!",     jp: "このバージョンを取得済み！" },
  "ver.goCheckin":      { en: "Go check in at this shop →", th: "เดินทางไปเช็คอินร้านนี้ →", jp: "この店にチェックインしに行く →" },
  // -- QR Scanner --
  "qr.gpsRequired":     { en: "GPS must be enabled and allowed", th: "จำเป็นต้องเปิดและอนุญาต GPS", jp: "GPSの有効化と許可が必要です" },
  "qr.codePlaceholder": { en: "Enter QR code (if any)...", th: "พิมพ์รหัส QR Code (ถ้ามี)...", jp: "QRコードを入力（任意）..." },
  "qr.devMode":         { en: "Test mode (Dev GPS Simulator):", th: "โหมดทดสอบ (Dev GPS Simulator):", jp: "テストモード（GPSシミュレーター）：" },
  "qr.devModeDesc":     { en: "Tap the button below to simulate arriving at the real location and scanning its QR code.", th: "กดปุ่มด้านล่างเพื่อจำลองว่าคุณเดินทางไปถึงพิกัดสถานที่จริงและสแกน QR Code ประจำจุดนั้น", jp: "下のボタンで、実際の地点に到着してQRコードをスキャンした状態を再現します。" },
  "qr.scanHere":        { en: "Scan using your current location", th: "สแกนด้วยพิกัดปัจจุบันของคุณ", jp: "現在地でスキャン" },
  "qr.simulateHere":    { en: "Simulate being here and check in", th: "จำลองพิกัดให้อยู่ที่นี่และเช็คอินสำเร็จ", jp: "この地点にいると想定してチェックイン" },
  // -- Store Rules --
  "rules.title":        { en: "Set shop rules and restrictions", th: "กำหนดกฎระเบียบและข้อห้ามประจำร้าน", jp: "店舗のルール・禁止事項を設定" },
  "rules.subtitle":     { en: "Pick a preset rule (e.g. no photos) or add your own", th: "เลือกกฎสำเร็จรูป (เช่น ห้ามถ่ายรูป) หรือเพิ่มข้อกำหนดเฉพาะร้านของคุณ", jp: "定型ルール（撮影禁止など）を選ぶか、独自のルールを追加" },
  "rules.addCustom":    { en: "Add your own rule / restriction:", th: "เพิ่มข้อกำหนด / กฎระเบียบพิเศษพิมพ์เอง:", jp: "独自のルール・規定を追加：" },
  "rules.customPlaceholder": { en: "e.g. No sandals, please wear socks, minimum order 100 THB", th: "เช่น ห้ามใส่รองเท้าแตะ, กรุณาสวมถุงเท้า, สั่งขั้นต่ำ 100 บาท", jp: "例：サンダル不可、靴下着用、最低注文100バーツ" },
  "rules.notePlaceholder": { en: "Additional note (optional)", th: "คำอธิบายเพิ่มเติม (ถ้ามี)", jp: "補足説明（任意）" },
  "rules.removeItem":   { en: "Remove this item",      th: "ลบข้อนี้",                   jp: "この項目を削除" },
  // -- Jigsaw Board --
  "jig.manageTitle":    { en: "Go to jigsaw quest management", th: "ไปยังหน้าจัดการเควสต์จิ๊กซอว์", jp: "ジグソークエスト管理へ" },
  "jig.manageQuests":   { en: "Manage Quests (Admin)", th: "จัดการเควสต์ (Admin)",       jp: "クエスト管理（管理者）" },
  "jig.openScanner":    { en: "Open QR + GPS scanner",  th: "เปิดกล้องสแกน QR + GPS",    jp: "QR・GPSスキャナーを開く" },
  "jig.board":          { en: "Jigsaw Board",           th: "กระดานภาพจิ๊กซอว์",         jp: "ジグソーボード" },
  "jig.reset":          { en: "Reset for testing",      th: "รีเซ็ตเพื่อทดสอบใหม่",      jp: "テスト用にリセット" },
  "jig.rewardCode":     { en: "Reward discount code",   th: "รหัสส่วนลดรางวัล",          jp: "特典割引コード" },
  "jig.gpsNeeded":      { en: "GPS required while scanning", th: "ต้องเปิด GPS ขณะสแกน",  jp: "スキャン時はGPSが必要" },
  "jig.goScan":         { en: "Go scan",                th: "ไปสแกน",                    jp: "スキャンへ" },
  "jig.hint":           { en: "Hint:",                  th: "คำใบ้:",                    jp: "ヒント：" },
  // -- Merchant Register --
  "merchant.subtitle":  { en: "Fill in your shop details for admin approval", th: "กรอกข้อมูลร้านค้าเพื่อส่งให้แอดมินพิจารณาอนุมัติสิทธิ์", jp: "管理者の承認のため店舗情報を入力してください" },
  "merchant.pending":   { en: "Your shop application is pending approval", th: "คำขอเปิดร้านค้าของคุณอยู่ระหว่างการรออนุมัติ", jp: "店舗申請は承認待ちです" },
  "merchant.rejected":  { en: "Your previous shop application was not approved", th: "คำขอเปิดร้านค้าก่อนหน้านี้ไม่ผ่านการอนุมัติ", jp: "前回の店舗申請は承認されませんでした" },
  "merchant.shopNamePlaceholder": { en: "e.g. Tokyo Ramen Bar, Kyoto Tea House", th: "เช่น Tokyo Ramen Bar, Kyoto Tea House", jp: "例：Tokyo Ramen Bar、Kyoto Tea House" },
  "merchant.fullNamePlaceholder": { en: "Full name", th: "ชื่อ-นามสกุล",                jp: "氏名" },
  "merchant.phonePlaceholder":    { en: "e.g. 081-234-5678", th: "เช่น 081-234-5678",   jp: "例：081-234-5678" },
  "merchant.prefecture": { en: "Prefecture",            th: "จังหวัด (Prefecture)",      jp: "都道府県" },
  "merchant.category":   { en: "Shop category",         th: "หมวดหมู่ร้านค้า",           jp: "店舗カテゴリー" },
  "merchant.fileHint":   { en: "Supports PDF, PNG, JPG up to 10MB", th: "รองรับ PDF, PNG, JPG ขนาดไม่เกิน 10MB", jp: "PDF・PNG・JPG、10MBまで対応" },
  "merchant.cancelFail": { en: "Could not cancel the request. Please try again.", th: "ไม่สามารถยกเลิกคำขอได้ กรุณาลองใหม่อีกครั้ง", jp: "申請をキャンセルできませんでした。もう一度お試しください。" },
  // -- Notification Bell --
  "notif.bellTitle":    { en: "Notifications & request status", th: "การแจ้งเตือนและสถานะคำขอ", jp: "お知らせ・申請状況" },
  "notif.title":        { en: "Notifications",         th: "การแจ้งเตือน",              jp: "お知らせ" },
  "notif.newAnn":       { en: "Create new announcement", th: "สร้างประกาศใหม่",          jp: "新しいお知らせを作成" },
  "notif.announce":     { en: "Announce",               th: "ประกาศ",                    jp: "お知らせ" },
  "notif.readAll":      { en: "Mark all as read",       th: "อ่านทั้งหมด",               jp: "すべて既読にする" },
  "notif.deleteAll":    { en: "Delete all",             th: "ลบทั้งหมด",                 jp: "すべて削除" },
  "notif.deleteOne":    { en: "Delete notification",    th: "ลบการแจ้งเตือน",            jp: "お知らせを削除" },
  "notif.confirmDeleteAnn": { en: "Delete this announcement?", th: "คุณต้องการลบประกาศนี้ใช่หรือไม่?", jp: "このお知らせを削除しますか？" },
  "notif.merchantPending":  { en: "Shop application pending approval", th: "คำขอเปิดร้านค้ารอการอนุมัติ", jp: "店舗申請は承認待ち" },
  "notif.merchantRejected": { en: "Shop application was not approved", th: "คำขอเปิดร้านค้าไม่ผ่านการอนุมัติ", jp: "店舗申請は承認されませんでした" },
  "notif.viewEdit":     { en: "View details / edit",    th: "ดูรายละเอียด / แก้ไขข้อมูล", jp: "詳細を見る・編集" },
