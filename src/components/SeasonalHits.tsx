  ...
  return (
    <div className="w-full min-w-0">
      ...
      <Carousel
        ...
        desktopClassName="md:grid md:grid-cols-2"
        showDots={false}
        renderItem={(p: any) => {
          const shopName = p.shop_name || p.name || "Unknown Shop";
          return (
            ...
            >
              <div className="h-24 relative" style={{ background: C.accentSoft }}>
                <span
              ...
              <div className="p-3">
                <h3 className="text-xs font-black truncate" style={{ color: C.ink }}>{shopName}</h3>
                <span className="text-[10px] text-[#8A7870]">{p.region || p.prefecture}</span>
                <div className="flex items-center gap-1 mt-1">
                  <Star size={10} fill={C.gold} color={C.gold} />
                  <span className="text-[11px] font-bold" style={{ color: C.ink }}>{(p.rating ?? 0).toFixed(1)}</span>
                  <span className="text-[9px] text-[#8A7870]">({p.reviews_count ?? 0})</span>
                </div>
              </div>
            </div>
          );
